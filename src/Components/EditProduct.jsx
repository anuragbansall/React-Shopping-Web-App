    ...
    return (
        <div className='min-h-screen w-full flex flex-col items-center p-8'>
            <div className='bg-slate-00 w-[50rem] max-w-full h-fit'>
                <h2 className='text-3xl font-semibold my-4'>Edit Product</h2>
                <form className='flex flex-col gap-4' onSubmit={submitFormDataHandler}>
                    ...
                    <div className='flex gap-4 md:gap-8 flex-col md:flex-row'>
                        ...
                        <input required type="text" name="price" className='w-full p-2 rounded-md bg-zinc-100' placeholder='price' value={newProductData.price || ""} onChange={handleInputData} />
                    </div>
                    <div className='flex gap-4 md:gap-8 flex-col md:flex-row'>
                    ...
                    <button className='self-start border md:border-2 border-blue-400 text-blue-400 hover:bg-blue-400 hover:text-white duration-200 px-8 py-1 text-lg md:text-xl md:font-medium rounded-md text-center'>Edit Product</button>
                </form>
            </div>
        </div>
    )
